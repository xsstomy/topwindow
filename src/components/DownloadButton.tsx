...
export default function DownloadButton({
  ...
  useEffect(() => {
    const loadInfo = async () => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : 'Failed to load download info'
        );
        setState('error');
      }
    };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      googleAnalytics.trackEvent('download_attempt', {
      ...
      const fileSize = versionInfo?.builds.macos?.size
        ? parseInt(versionInfo.builds.macos.size.replace(/[^\d.]/g, '')) *
          1024 *
          1024 // Convert MB string to bytes
        : undefined;
      googleAnalytics.trackDownload(versionInfo?.version || version, fileSize);
      ...
      googleAnalytics.trackEvent('download_error', {
